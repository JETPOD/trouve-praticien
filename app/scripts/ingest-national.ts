...
// HDF files in DATA_DIR root + all national CSVs auto-discovered in DATA_DIR/national/
const HDF_AMELI_FILES = [
  "ameli_Aisne.csv",
  "sample_59.csv",
  "ameli_Oise.csv",
  "sample_62.csv",
...
function discoverNationalFiles(): string[] {
  const nationalDir = path.join(DATA_DIR, "national");
  if (!fs.existsSync(nationalDir)) return [];
  return fs
    .readdirSync(nationalDir)
    .filter((f) => f.toLowerCase().endsWith(".csv"))
    .map((f) => path.join("national", f))
    .sort();
}
...
// Map French labels (use_labels=true) to technical names used by the HDF sample CSVs.
const HEADER_ALIAS_MAP: Record<string, string> = {
  "civilité": "civilite",
  "nom du professionel": "nom",
  ...
  "mode d’exercice particulier": "exercice_particulier",
  "mode d'exercice particulier": "exercice_particulier",
  "nature d’exercice": "nature_exercice",
  "nature d'exercice": "nature_exercice",
  "convention": "convention",
  "sesam vitale": "sesam_vitale",
...
function normalizeHeader(h: string[]): string[] {
  return h.map((name) => {
    ...
    return HEADER_ALIAS_MAP[k] || name.trim();
  });
}
...
async function processAmeliFile(
  ...
  for await (const rawLine of rl) {
    ...
    if (!header) {
      header = normalizeHeader(parseCSVLineSemi(line));
      continue;
    }
    ...
    if (!row.nom || !row.libelle_profession) continue;
...
async function main() {
  ...
  const ALL_AMELI_FILES = [...HDF_AMELI_FILES, ...discoverNationalFiles()];
  console.log(`[ingest-v2] Ameli files to process: ${ALL_AMELI_FILES.length}`);
  for (const fname of ALL_AMELI_FILES) {
  ...
  // Bigger SQLite cache for the bulk insert phase
  ...
  {
    const all = [...practitioners.values()];
    const CHUNK = 5000;
    for (let i = 0; i < all.length; i += CHUNK) {
      const slice = all.slice(i, i + CHUNK);
      insertMany(slice);
      if ((i / CHUNK) % 10 === 0)
        console.log(`[ingest-v2] inserted ${i + slice.length}/${all.length}`);
    }
  }
